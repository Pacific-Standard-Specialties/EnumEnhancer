...
namespace PSS.EnumEnhancers.ExtensionMethods.Tests.Generators.EnumExtensions;
...
public class EnumExtensionMethodsIncrementalGeneratorTests
{
  ...
  /// </summary>
  [SuppressMessage("ReSharper", "CollectionNeverQueried.Local", Justification = "Yes it is... By tests...")]
  ...
  {
    return
      GetAssemblyExtendedEnumTypeAttributes()
       .Select(
               static attr =>
                 new TestCaseData(attr)
                 {
                   TestName =
                     $"{nameof(AssemblyExtendedEnumTypeAttribute_EnumHasGeneratorAttribute)}({attr.EnumType.Name},{attr.ExtensionClass.Name})",
  ...
  [Test]
  [Category("Attributes")]
  public void AssemblyExtendedEnumTypeAttribute_ExtensionClassHasExpectedReverseMappingAttribute(
    [ValueSource(nameof(GetAssemblyExtendedEnumTypeAttributes))] AssemblyExtendedEnumTypeAttribute attr)
  {
    Assume.That(attr, Is.Not.Null);
  ...
  [Test]
  [Category("Attributes")]
  public void ExtendedEnum_AssemblyHasMatchingAttribute(
    [ValueSource(nameof(GetExtendedEnum_EnumData))] EnumData enumData)
  {
  ...
  [Test]
  public void ExtendedEnum_FastIsDefinedFalse_DoesNotHaveFastIsDefined(
    [ValueSource(nameof(GetExtendedEnumTypes_FastIsDefinedFalse))] EnumData enumData)
  {
    Assume.That(enumData.EnumType.IsEnum);
    ...
    Assume.That(enumData.GeneratorAttribute,
                Is.EqualTo(enumData.EnumType.GetCustomAttribute<GenerateEnumExtensionMethodsAttribute>()));
  ...
  [Test]
  public void ExtendedEnum_FastIsDefinedTrue_HasFastIsDefined(
    [ValueSource(nameof(GetExtendedEnumTypes_FastIsDefinedTrue))] EnumData enumData)
  {
    Assume.That(enumData.EnumType, Is.Not.Null);
    Assume.That(enumData.EnumType.IsEnum);
    Assume.That(enumData.EnumType, Has.Attribute<GenerateEnumExtensionMethodsAttribute>());
  ...
  private static void Initialize()
  {
    ...
    try
    {
      ...
      foreach (Type type in allAssemblyTypes.Where(static t => t.IsClass &&
                                                               t.IsDefined(typeof(ExtensionsForEnumTypeAttribute<>))))
      {
    ...
    static bool IsDefinedEnum(Type t)
    {
      return t is
             {
               IsEnum: true,
    ...
    static void EnumExtensionClasses_OnCollectionChanged(object? sender, NotifyCollectionChangedEventArgs e)
    {
      ...
      {
        if (extensionClassType.GetCustomAttribute(typeof(ExtensionsForEnumTypeAttribute<>), false) is not
            IExtensionsForEnumTypeAttributes { EnumType.IsEnum: true } extensionForAttribute)
        {
          continue;
        }
  ...
  private static void AllEnumTypes_CollectionChanged(object? sender, NotifyCollectionChangedEventArgs e)
  {
    if (e.Action is not NotifyCollectionChangedAction.Add and not NotifyCollectionChangedAction.Replace ||
        e.NewItems is null)
    {
      return;
    }
    ...
    static EnumData CreateNewEnumData(Type tEnum, GenerateEnumExtensionMethodsAttribute attr)
    {
      return new(tEnum, attr);
    }

    static EnumData UpdateGeneratorAttributeProperty(Type tEnum, EnumData data,
                                                     GenerateEnumExtensionMethodsAttribute attr)
    {
    ...
    IExtensionsForEnumTypeAttributes? ExtensionForEnumTypeAttribute = null)
  {
    ...
    public IExtensionsForEnumTypeAttributes? ExtensionForEnumTypeAttribute { get; set; } =
      ExtensionForEnumTypeAttribute;
